...
///
/// It lives in this package, rather than beside the script that first needed
/// it, because there is now more than one entry point: the e2e rehearsal and
/// the served API both have to find the same file the same way.

import { existsSync, readFileSync } from "node:fs";
import { dirname, resolve } from "node:path";

/// Walk up looking for the workspace marker. Bounded, because an unbounded
/// walk on a machine with no marker at all climbs to `/`.
