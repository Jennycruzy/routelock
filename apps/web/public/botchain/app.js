...
const ROUTELOCK_BUILD = window.ROUTELOCK_BUILD === "botchain" ? "botchain" : "xlayer";
const ROUTELOCK_BASE_PATH = String(window.ROUTELOCK_BASE_PATH || "").replace(/\/$/, "");

// Each hosted build has one chain. The X Layer build is the default; the BOT
// build sets ROUTELOCK_BUILD and its own API base path in its entrypoint.
let selectedNetwork = ROUTELOCK_BUILD === "botchain" ? "botchain_testnet" : "xlayer_mainnet";
...
const activeServiceRoles = { xlayer: "buyer", botchain: "buyer" };
...
function updateSelection(networkKey) {
  ...
  document.querySelectorAll("[data-select-network]").forEach((button) => {
    const badgeNetwork = NETWORKS[button.dataset.selectNetwork];
    const active = badgeNetwork?.group === network.group;
    button.classList.toggle("active", active);
    button.setAttribute("aria-pressed", String(active));
  });

  document.querySelectorAll("[data-network-card]").forEach((card) => {
    card.classList.toggle("active", card.dataset.networkCard === network.group);
    const connect = card.querySelector("[data-connect-network]");
    if (connect && card.dataset.networkCard === network.group) {
      connect.dataset.connectNetwork = networkKey;
      connect.innerHTML = `Connect to ${esc(network.shortName)} <span aria-hidden="true">↗</span>`;
    }
  });

  $("current-network").textContent = network.group === "xlayer" ? "Carbon retirement" : "AI compute";
  updateServiceButtonLabels();
}

function updateServiceButtonLabels() {
  document.querySelectorAll("[data-connect-network]").forEach((button) => {
    const network = NETWORKS[button.dataset.connectNetwork];
    if (!network) return;
    const label = network.group === "xlayer" ? "Open carbon retirement" : "Open compute service";
...
function showServiceView(service) {
  const view = $(service === "xlayer" ? "xlayer-view" : "botchain-view");
  if (!view) return;
  ...
  view.hidden = false;
  setServiceRole(service, activeServiceRoles[service] || "buyer");
  if (service === "xlayer") void loadMerchantData();
  renderBotCapability();
  requestAnimationFrame(() => view.scrollIntoView({ behavior: "smooth", block: "start" }));
}

function openServiceView(service, networkKey) {
  const network = networkKey || (service === "xlayer" ? "xlayer_mainnet" : "botchain_testnet");
  updateSelection(network);
  if (location.hash !== `#${service}`) location.hash = service;
  else showServiceView(service);
}

function routeFromHash() {
  const hash = location.hash.replace(/^#/, "");
  if (hash === "xlayer" || hash === "botchain") showServiceView(hash);
  else showLanding();
}

function setServiceRole(service, role) {
  if (service !== "xlayer" && service !== "botchain") return;
  ...
  // Only role panels are toggled here. The service section itself must remain
  // visible; including `#xlayer-view`/`#botchain-view` in this selector made a
  // provider tab hide its own page and left only the header and footer visible.
...
async function switchNetwork(networkKey) {
  ...
  try {
    ...
    setMessage(network.group === "xlayer" ? "You are ready to start carbon retirement." : "You are viewing the AI compute service.");
    ...
    if (error?.code === 4902) {
      try {
        ...
        setMessage(network.group === "xlayer" ? "Live X Layer was added. You are ready to start carbon retirement." : "The compute service network was added to your wallet.");
        updateWalletUi(await walletChainId());
        return;
      } catch (addError) {
...
async function api(path, options = {}) {
  const endpoint = `${ROUTELOCK_BASE_PATH}${path}` || "/";
...
async function startConsumerOrder() {
  ...
  try {
    ...
    setConsumerMessage("Checking the request and current carbon offer…", "pending");
...
function renderConsumerCapability() {
  ...
  const isBot = consumerCapabilities?.compute?.supported === true;
  ...
  element.innerHTML = enabled
    ...
    : `<span class="status-dot ${productStatus ? "live" : "testing"}"></span> <strong>${isBot ? "AI compute is being completed on BOT Chain." : "Carbon retirement is live on X Layer Mainnet."}</strong> ${isBot ? "This page is showing the compute service." : isMainnet ? "The service is live, but a purchase is not available at this moment." : "This is a read-only test view. Choose Live X Layer above to start a real retirement."}`;
  $("carbon-panel-state").textContent = enabled ? "READY" : isBot ? "COMPUTE VIEW" : productStatus ? "READ-ONLY" : "NOT AVAILABLE";
  $("carbon-form").hidden = !enabled;
  if (!enabled) $("carbon-empty").textContent = productStatus
    ? isBot ? "Choose Live X Layer above to start a carbon retirement." : "A backed carbon offer is not available at this moment."
    : "Choose Live X Layer above to start a carbon retirement.";
}
...
function renderMerchantOffers() {
  ...
  if (!offerings.length) {
    list.innerHTML = `<p class="empty-state">No offer owned by this wallet was found. Create one on the right, or connect the issuer wallet that created an existing offer.</p>`;
...
async function loadMerchantData() {
  ...
  const network = selected();
  if (network.group !== "xlayer") {
    capability.className = "consumer-capability warning";
    capability.innerHTML = `<span class="status-dot testing"></span> <strong>Provider offer controls are on the X Layer service page.</strong> BOT Chain has its own provider run and is shown separately. `;
    return;
  }
  if (!walletAccount) {
    capability.className = "consumer-capability warning";
    capability.innerHTML = `<span class="status-dot testing"></span> <strong>Connect the issuer wallet to manage offers.</strong> The page will read live class backing and let that wallet approve every change.`;
  ...
  try {
    ...
    $("merchant-issuer-state").textContent = registered ? paused ? "PAUSED" : "REGISTERED" : "NOT REGISTERED";
    capability.className = `consumer-capability ${registered && !paused ? "ready" : "warning"}`;
    capability.innerHTML = registered && !paused
      ? `<span class="status-dot live"></span> <strong>This wallet can publish offers on X Layer Mainnet.</strong> Prices and collateral are read from the live contracts; your wallet approves every write.`
...
function renderBotCapability() {
  const element = $("bot-capability");
  const lane = consumerCapabilities?.lanes?.compute;
  if (!element || !lane) return;
  const active = lane.status === "active" && consumerCapabilities?.compute?.active === true;
  const configured = lane.providerConfigured === true;
  const detail = lane.reason || "one real provider lease and public result are still required before customer payment opens";
  element.className = `consumer-capability ${active ? "ready" : "warning"}`;
  element.innerHTML = active
    ? `<span class="status-dot live"></span> <strong>BOT Chain compute is live.</strong> The committed Akash ingress proof is available from ${esc(lane.chainName || "the selected BOT Chain deployment")}.`
    : configured
      ? `<span class="status-dot testing"></span> <strong>BOT Chain compute is configured but not active.</strong> ${esc(detail)}.`
      : `<span class="status-dot testing"></span> <strong>BOT Chain compute is not configured.</strong> ${esc(detail)}.`;
}
...
async function createMerchantOffer(event) {
  ...
  try {
    ...
    if (!registered) throw new Error("this wallet is not registered as an issuer on X Layer");
...
async function fundMerchantOffer() {
  ...
  try {
    ...
    // Refresh both sides. The customer selector may have been rendered before
    // this provider transaction and otherwise keeps the old unavailable state.
    await loadConsumerData();
  } catch (error) {
...
async function loadLiveStatus() {
  ...
  try {
    ...
    details.innerHTML = `<div class="live-facts">
      <span class="live-fact">Service <strong>${esc(capabilities.compute?.supported ? "AI compute" : "Carbon retirement")}</strong></span>
      <span class="live-fact">${capabilities.compute?.supported ? "Full proof is being completed" : proofSummary}</span>
